...
<select class="form-select" name="category" aria-label="Default select example">
    <option selected value="{{ old('category') ?? $post->category ?? ''}}">Категорія</option>
  ...
  <div class="form-group">
    ...
    <input class="form-control" id="articul" name="articul" type="number" value="{{ old('articul') ?? $post->articul ?? ''}}">
</div>
<label for="slug">Адреса сторінки</label>
<input class="form-control" type="text" id="slug" name="slug" placeholder="Адреса сторінки для SEO приклад: (swimtrainer-red)" value="{{ old('slug') ?? $post->slug ?? ''}}" required>
<div class="form-group">
...
<label class="form-label" for="price">Ціна товару</label>
  <input class="form-control" id="price" name="price" type="number" value="{{ old('price') ?? $post->price ?? ''}}">
  <span class="helper-text" data-error="Введіть ціну"></span>
</div>
<div class="form-group">
    <label class="form-label" for="img">Головне фото</label>
  <input type="file" name="img" class="form-control m-2" value="{{ old('img') ?? $post->img ?? ''}}">
</div>
...
<div class="form-group">
    ...
    <textarea class="form-control" id="features" name="features" cols="30" rows="20">{{ old('features') ?? $post->features ?? ''}}</textarea>
</div>
<div class="form-group">
...
<div class="form-group">
    ...
    <input type="text" class="form-control" name="carousel" placeholder="Посилання на відео" value="{{ old('carousel') ?? $post->carousel ?? ''}}">
</div>
<div class="form-group">
